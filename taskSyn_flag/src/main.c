...
int main(void)
{
    ...
    OSFlagCreate((OS_FLAG_GRP *)&synFlags,
                 (CPU_CHAR *)"Sychronization Flags",
                 (OS_FLAGS)0,
                 (OS_ERR *)&err);
...
static void LED3Task(void *p_arg)
{
    ...
    while (DEF_TRUE)
    {
        ...
        OSFlagPost((OS_FLAG_GRP *)&synFlags, //Signal LED3 has toggled once by setting SYN_LED3_OK bit 1
                   (OS_FLAGS)SYN_LED3_OK,
...
static void LED4Task(void *p_arg)
{
    ...
    while (DEF_TRUE)
    {
        ...
        OSFlagPost((OS_FLAG_GRP *)&synFlags, //Signal LED4 has toggled once by setting SYN_LED4_OK bit 1
                   (OS_FLAGS)SYN_LED4_OK,
                   (OS_OPT)OS_OPT_POST_FLAG_SET + OS_OPT_POST_NO_SCHED,
                   (OS_ERR *)&err);
...
static void SendStringTask(void *p_arg)
{
    ...
    while (DEF_TRUE)
    {
        OSFlagPend((OS_FLAG_GRP *)&synFlags, //Wait for both bits to be set
                   (OS_FLAGS)SYN_LED3_OK + SYN_LED4_OK,
                   (OS_TICK)0,
